...
-export([send/2, status/1, stop/1, await/2, blackboard/1, interrupt/2, resume/2, cancel/1]).
...
-spec cancel(gakudan:run_id()) -> ok | {error, not_found}.
cancel(RunId) ->
    with_run(RunId, fun(#{run_statem := Pid}) -> gakudan_run_statem:cancel(Pid) end).
